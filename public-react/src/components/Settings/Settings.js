...
class Settings extends PureComponent {
  ...
  render() {
    const { userHue, username } = this.props;
      ...
      <section className="settings">
        ...
        >
          <svg className="settings-cog icon icon-cog">
            <path 
              ...
              stroke={`hsl(${userHue}, 86%, 88%)`}
              fill={`hsl(${userHue}, 86%, 88%)`}
            />
          </svg>
        </div>
        <Modal open={modalIsOpen}>
          <form
            ...
            style={{
              backgroundColor: `hsl(${previewHue || userHue}, 40%, 93%)`,
              color: `hsl(${previewHue || userHue}, 86%, 27%)`,
            }}
          >
            <HuePicker 
              color={{ h: previewHue || userHue, s: 1, l: .5 }}
